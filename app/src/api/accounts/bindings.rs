...
#[allow(unused_imports)]
use crate::domain::account::bindings::{
    BaseRoleBinding,
    binding::{RoleBinding, RoleBindingsResponse},
    clusterbinding::{ClusterRoleBinding, ClusterRoleBindingsResponse},
};
...
pub async fn get_rolebindings(namespace_name: Option<String>) -> Result<Vec<RoleBinding>, ServerFnError> {
    let response = kube_api_rbac_request("rolebindings".to_string()).await?;
    let items = serde_json::from_str::<RoleBindingsResponse>(&response)?.items
        .into_iter()
...
pub async fn get_clusterrolebindings() -> Result<Vec<ClusterRoleBinding>, ServerFnError> {
    let response = kube_api_rbac_request("clusterrolebindings".to_string()).await?;
    Ok(serde_json::from_str::<ClusterRoleBindingsResponse>(&response)?.items)
}
