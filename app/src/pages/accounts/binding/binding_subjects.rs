...
#[component]
pub fn RoleBindingSubjectsComponent(namespace_name: String, binding_name: String) -> impl IntoView {
...
fn update_page(
    ...
    if binding_name.is_disposed() || namespace_name.is_disposed() {
        return;
    }
    ...
    spawn_local(async move {
        ...
        let binding = bindings_api::get_rolebindings(selected_value)
            .await
            .unwrap_or_default()
            .iter()
            .find(|sc| sc.metadata.name == binding_name)
            .cloned()
            .unwrap_or_default();
