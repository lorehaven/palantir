...
#[component]
pub fn SecretInfoComponent(namespace_name: String, secret_name: String) -> impl IntoView {
...
fn update_page(
    ...
    if secret_name.is_disposed() || namespace_name.is_disposed() {
        return;
    }
    ...
    spawn_local(async move {
        ...
        let secret = secrets_api::get_secrets(selected_value)
            .await
            .unwrap_or_default()
            .iter()
            .find(|sc| sc.metadata.name == secret_name)
            .cloned()
            .unwrap_or_default();
