...
fn update_page(
    ...
    if namespace_name.is_disposed() || serviceaccount_name.is_disposed() { return; }
    ...
    spawn_local(async move {
        ...
        let sa = serviceaccounts_api::get_serviceaccounts(selected_value).await
            .unwrap_or_default()
            .iter().find(|sc| sc.metadata.name == serviceaccount_name)
            .cloned()
            .unwrap_or_default();

        serviceaccount_data.set(vec![
