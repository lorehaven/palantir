...
#[component]
pub fn StorageClassInfoComponent(storageclass_name: String) -> impl IntoView {
...
fn update_page(
    ...
    if storageclass_name.is_disposed() {
        return;
    }
    ...
    spawn_local(async move {
        let storageclass = storage_api::get_storageclasses()
            ...
            .unwrap_or_default()
            .iter()
            .find(|sc| sc.metadata.name == storageclass_name)
            .cloned()
            .unwrap_or_default();

        storageclass_data.set(
            vec![
