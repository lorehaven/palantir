...
#[component]
pub fn JobStatsComponent(
    namespace_name: RwSignal<String>,
    resource_name: RwSignal<String>,
) -> impl IntoView {
    let jobs_ready = RwSignal::new(String::new());
...
fn update_page(
    ...
    spawn_local(async move {
        ...
        let job = jobs_api::get_jobs(namespace_name.clone())
            .await
            .unwrap_or_default()
            .into_iter()
        ...
        let deployment_pods = pods_api::get_pods(namespace_name.clone(), None)
            ...
            .collect::<Vec<_>>();
        jobs_ready.set(format!("{} / {}", job.status.ready, job.status.succeeded));
        ...
        <Wrapper>
            <WrapperSlot slot>
                <div class="card-container dcc-3">
                    <CardString
                        label=""
                        label_add="Active / Succeeded"
                        value=jobs_ready.get() />
                    <CardCircle
                        label="Pod CPU usage"
