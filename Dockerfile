...
# wants here.)
RUN apk update && apk add --no-cache build-base openssl openssl-dev openssl-libs-static musl-dev bash curl npm libc-dev
...
# quench-web generates its own CSS/JS/favicon at runtime (see
# `app/src/shell.rs`) into the same `dist/assets` tree - this only needs to
# add palantir's own hand-written styling (`styles/*.scss`) to it.
RUN mkdir -p dist/assets/css && sass --no-source-map styles/main.scss dist/assets/css/palantir.css
...
# ALLOW_IN_MEMORY_DB are left unset here on purpose - the deployment manifest
# injects those at runtime, same as every other forge service (see
# homecloud's palantir overlay).
